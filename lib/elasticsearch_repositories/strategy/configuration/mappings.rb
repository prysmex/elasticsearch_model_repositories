  ...
  module Strategy
    module Configuration
      ...
      class Mappings
        ...
        # @private
        TYPES_WITH_EMBEDDED_PROPERTIES = %w[object nested].freeze
        ...
        def indexes(field_name, definition = {}, &)
          @mapping[field_name] = definition

          if block_given?
            @mapping[field_name][:type] ||= 'object'
            properties = if TYPES_WITH_EMBEDDED_PROPERTIES.include?(@mapping[field_name][:type].to_s)
              :properties
            else
              :fields
            end
        ...
        # @param [Proc] &block
        # @return [void]
        def register_dynamic_properties_method(method_name, &)
          define_singleton_method(method_name, &) if block_given?

          methods = instance_variable_get(:@dynamic_properties_methods)
          methods.push(method_name).uniq
          instance_variable_set(:@dynamic_properties_methods, methods)
        end

        # Serialize into hash. If any methods where registed using #register_dynamic_properties_method,
        # arguments should be passed or use dynamic_properties_methods_to_skip param
        #
        # @param [Array] dynamic_properties_methods_to_skip
        ...
        def to_hash(dynamic_properties_methods_to_skip: [], **dynamic_properties_methods_args)
          # static properties
          ...
          dynamic_properties_methods.each do |method_name|
            next if dynamic_properties_methods_to_skip.include?(method_name)

            public_send(method_name, mappings_hash, *dynamic_properties_methods_args[method_name])
          end
        ...
        def to_flattened_hash
          ...
          flattened = flattened.select { |_k, v| datatypes.include?(v) }
